    ...
    <div style={{ backgroundColor: '#ffffff', color: '#111827', minHeight: '100vh', width: '100%', overflowX: 'hidden' }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
          <img src="/images/modes/executes.jpg" alt={t("auto.page.executes")} style={{ width: '40px', height: '40px', borderRadius: '12px' }} />
          <h2 style={{ fontSize: '1.2rem', fontWeight: 600, margin: 0, letterSpacing: '1px' }}>{t("auto.page.executes")}</h2>
        </div>
      ...
      <div style={{ position: 'relative', height: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '0 20px' }}>
        ...
        >
          {t("auto.page.flawless_site_takes")} <br />{t("auto.page.powered_by_automated_utility_d")}
                          </motion.p>
      </div>
      ...
      <div style={{ width: '100%' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'row', minHeight: '80vh' }}>
          <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} style={{ flex: 1, background: '#f8fafc', padding: '10%', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
            <h2 style={{ fontSize: '4rem', fontWeight: 800, color: '#0ea5e9', marginBottom: '24px', letterSpacing: '-1px', lineHeight: 1.1 }}>{t("auto.page.pro_utility")}<br/>{t("auto.page.perfect_timing")}</h2>
            <p style={{ fontSize: '1.3rem', color: '#64748b', lineHeight: 1.6 }}>{t("auto.page.the_server_throws_perfectly_ti")}</p>
          </motion.div>
        ...
        <div style={{ display: 'flex', flexDirection: 'row-reverse', minHeight: '80vh' }}>
          <motion.div initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} style={{ flex: 1, background: '#0f172a', padding: '10%', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
            <h2 style={{ fontSize: '4rem', fontWeight: 800, color: '#fff', marginBottom: '24px', letterSpacing: '-1px', lineHeight: 1.1 }}>{t("auto.page.defend_the_hold")}</h2>
            <p style={{ fontSize: '1.3rem', color: '#94a3b8', lineHeight: 1.6 }}>{t("auto.page.ct_players_spawn_in_randomized")}</p>
          </motion.div>
