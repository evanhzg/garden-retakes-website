...
import { compare, pickDaily, todayKey, msUntilNextDay, seededShuffle, type HeadshotPlayer } from "@/scripts/headshotRules";
import { useHeadshotPool } from "@/components/games/headshot/useHeadshotPool";
import { SearchBox, GuessGrid, Legend, shareSquares, flagOf, countryName, rolesLabel, type GuessRow } from "@/components/games/headshot/GuessBoard";
...
export default function HeadshotPage() {
  ...
  const rows: GuessRow[] = useMemo(() => {
    ...
    return guessIds
      ...
      .filter(Boolean)
      .map((p) => ({ player: p as HeadshotPlayer, result: compare(p as HeadshotPlayer, target, `${date}T12:00:00Z`) }));
  ...
  const shareText = useMemo(() => {
    ...
    const body = rows.slice(0, 12).map((r) => shareSquares(r.result)).join("\n");
      ...
      <main className="hs-main">
        {error ? (
          ...
          <>
            ...
            <GuessGrid rows={[...rows].reverse()} lang={lang} onDate={`${date}T12:00:00Z`} />
            {rows.length === 0 && <Legend lang={lang} />}
          </>
        )}
      </main>
