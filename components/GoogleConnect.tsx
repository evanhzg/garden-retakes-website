...
export default function GoogleConnect() {
  ...
  // Surface the ?google=… result from the callback redirect, then clean the URL.
  ...
  const unlink = async () => {
    setBusy(true);
    try { await fetch("/api/auth/google/unlink", { method: "POST" }); await load(); }
  ...
  return (
    <div className="google-connect">
      <div className="google-connect-head">
        <span className="google-glyph" aria-hidden>
      ...
      {notice && <div className="google-notice">{notice}</div>}

      {status.linked ? (
        <div className="google-linked">
          <div className="google-user">
            {status.avatar ? <img src={status.avatar} alt="" /> : <span className="google-user-fallback">{(status.name || "?").charAt(0).toUpperCase()}</span>}
        ...
        <a
          className={`google-btn ${status.configured ? "" : "disabled"}`}
